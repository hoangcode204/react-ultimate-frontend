...
import { fetchAllBooksAPI } from "../services/api.service"; // Gọi API để lấy dữ liệu
...
const BookPage = () => {
  ...
  const loadBooks = async () => {
    ...
    const res = await fetchAllBooksAPI(current, pageSize); // Chờ kết quả trả về từ API
  ...
  return (
    <div style={{ padding: "20px" }}>
      <BookForm loadBooks={loadBooks} /> {/* Form thêm/sửa sách */}
      <BookTable
        dataBooks={dataBooks}
        loadBooks={loadBooks}
        current={current}
